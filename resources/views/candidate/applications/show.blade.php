...
@section('content')
  <div class="container">
    <div class="d-flex justify-content-between align-items-center mb-4">
      ...
      ><i class="bi bi-arrow-left"></i> Kembali ke Profil</a>
    </div>
        ...
        <div class="card mb-4 shadow-sm">
          ...
          <div class="card-body">
            <p class="text-muted">Batas Lamaran:
              {{ $application->jobVacancy->deadline_at ? \Carbon\Carbon::parse($application->jobVacancy->deadline_at)->isoFormat('D MMMMWriteHeader') : 'Tidak ditentukan' }}
            </p>
            <hr>
            ...
            >{{ $application->jobVacancy->description }}</div>
          </div>
        </div>
        ...
        <div class="card shadow-sm">
              ...
              <li class="list-group-item d-flex justify-content-between align-items-center">
                ...
                <div>
                  @if ($uploadedDocumentsMap->has($docName))
                    ...
                    <a
                      href="{{ asset('storage/' . $uploadedDocumentsMap[$docName]->file_path) }}"
                      target="_blank"
          ...
          @if ($application->status === 'belum_lengkap')
            <div class="card-footer bg-white">
              ...
              <form
                action="{{ route('candidate.applications.storeDocument', $application) }}"
                method="POST"
              ...
              >
                ...
                <div class="mb-3">
                  <label
                    for="file_surat_lamaran"
                    class="form-label"
                  >Surat Lamaran (PDF, JPG, PNG)</label>
                  <input
                    id="file_surat_lamaran"
                    class="form-control @error('surat_lamaran') is-invalid @enderror"
                    type="file"
                    name="surat_lamaran"
                  >
                  @error('surat_lamaran')
                ...
                <div class="mb-3">
                  <label
                    for="file_ijazah_transkrip"
                    class="form-label"
                  >Fotokopi Ijazah dan Transkrip Nilai</label>
                  <input
                    id="file_ijazah_transkrip"
                    class="form-control @error('ijazah_transkrip') is-invalid @enderror"
                    type="file"
                ...
                <div class="mb-3">
                  <label
                    for="file_skck"
                    class="form-label"
                  >Fotokopi SKCK (Opsional)</label>
                  <input
                    id="file_skck"
                    class="form-control @error('skck') is-invalid @enderror"
                    type="file"
                    name="skck"
                  >
                  @error('skck')
                ...
                <div class="mb-3">
                  <label
                    for="file_dokumen_pendukung"
                    class="form-label"
                  >Dokumen Pendukung (Opsional, multiple)</label>
                  <input
                    id="file_dokumen_pendukung"
                ...
                <p class="text-muted mb-0 mt-2 text-center"><small>Maks: 2MB per file. Format: PDF, JPG, JPEG, PNG (untuk
