    ...
    <div class="col-lg-4 mb-4">
      <div class="card border-0 shadow-sm">
        <div class="card-body text-center">
          {{-- Ikon ini sekarang otomatis berwarna hijau karena class 'text-primary' --}}
        ...
        <div class="list-group list-group-flush">
          {{-- ... (info data diri lainnya tidak berubah) ... --}}
          <div class="list-group-item">
            <small class="text-muted">Tempat, Tanggal Lahir</small>
            <p class="mb-0">{{ $user->candidateProfile?->place_of_birth ?? '-' }},
          ...
          <div class="list-group-item">
            ...
            <p class="text-capitalize mb-0">{{ $user->candidateProfile?->gender ?? '-' }}</p>
          </div>
          <div class="list-group-item">
          ...
          <div class="list-group-item">
            ...
            >{{ $user->candidateProfile?->address ?? '-' }}</p>
          </div>

          {{-- Tombol "Ubah" di sini otomatis berwarna hijau karena class 'btn-outline-primary' --}}
          <div class="list-group-item">
            <div class="d-flex justify-content-between align-items-center">
              <div>
                ...
                @if ($user->candidateProfile?->resume_path)
                  <p class="mb-0"><a
                      href="{{ asset('storage/' . $user->candidateProfile->resume_path) }}"
                      target="_blank"
                    >Lihat CV Terunggah <i class="bi bi-box-arrow-up-right"></i></a></p>
                @else
                  <p class="text-danger mb-0">Belum diunggah</p>
                @endif
              </div>
              <div>
                <a
                  href="{{ route('candidate.resume.edit') }}"
                  class="btn btn-sm btn-outline-primary"
                  title="{{ $user->candidateProfile?->resume_path ? 'Ubah CV Utama' : 'Upload CV Utama' }}"
                >
                  <i class="bi bi-pencil-square"></i> {{ $user->candidateProfile?->resume_path ? 'Ubah' : 'Upload' }}
                </a>
              </div>
            </div>
          </div>
        ...
        <div class="card-body">
          @if (!$user->candidateProfile?->is_complete || !$user->candidateProfile?->resume_path)
            <div class="alert alert-warning">
              ...
              Profil Anda belum sepenuhnya lengkap atau CV utama belum diunggah. Silakan lengkapi untuk dapat melamar
              pekerjaan.
            </div>
          @else
          ...
          <div class="d-flex flex-wrap gap-2">
            @if (!$user->candidateProfile?->is_complete)
              ...
              ><i class="bi bi-pencil-square me-1"></i> Lengkapi Data Diri</a>
            @endif
          </div>
        </div>
      </div>
        ...
        <div class="card-body">
          ...
          @else
            ...
            <div class="text-center">
              {{-- [UBAH] Ganti class 'btn-success' menjadi 'btn-primary' untuk konsistensi --}}
              <a
                href="{{ route('job_vacancies.index') }}"
                class="btn btn-primary"
              >Lihat Lowongan Tersedia</a>
            </div>
          @endif
        </div>
      </div>
